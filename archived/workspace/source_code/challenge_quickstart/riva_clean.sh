...
delete_docker_image() {
  ...
  if [[ $image_exists -eq 1 ]]; then
      read -r -p "Image $1 found. Delete? [y/N] " response
      if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
      ...
      echo "Image $1 has not been downloaded, or has already been deleted."
  fi
}
...
delete_docker_image $image_speech_api
delete_docker_image $image_init_speech
